...
plausible `dplyr` pipelines on the first try. The trouble is what happens next.
If you `eval(parse(text = ...))` that code in your own R session, the model
...
This tutorial builds a **data analyst agent** from scratch. By the end, you
will have an agent that accepts natural-language questions about a dataset,
generates R code to answer them, executes that code in a sandbox with
guardrails, draws on documentation via RAG, and logs every step with
full observability. Every code chunk runs without external API keys -- we
mock the LLM chat so you can follow along on any machine.
...
The foundation of any governed agent is **isolation**. When an LLM generates
...
But isolation alone is not enough. The agent needs *capabilities* -- the
...
result <- session$execute(sprintf('
  # Read the dataset using the file tool (returns a data frame for CSVs)
  data <- read_file(path = "%s/states.csv", format = "auto")
...
This gives you `$check_input()`, `$check_code()`, `$check_output()`, and
...
Retrieval gives you candidate chunks, but you still need to fit them into
...
A governed agent must be auditable. When something goes wrong -- a guardrail
blocks a legitimate query, or the model generates incorrect code -- you need
a trace that shows exactly what happened at each step. `securetrace`
...
the cost using built-in pricing tables. This is critical for monitoring
...
Guardrails are only as good as their accuracy. A guard that blocks
every input has perfect recall but zero precision. A guard that passes
everything has perfect precision but zero recall. `securebench` provides
the tooling to measure this tradeoff rigorously, so you can tune your
...
This tutorial walked through every layer of the secure-r-dev ecosystem:
...
The key design principle is **defense in depth**. No single layer is
sufficient on its own. The sandbox limits what code can do, but guardrails
prevent dangerous code from being generated in the first place. Output
filtering catches anything that slips through. Tracing ensures you can
audit every decision after the fact. And benchmarking gives you confidence
that your guards actually work.
